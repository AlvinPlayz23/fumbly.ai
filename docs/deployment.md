...
Fumbly AI CLI project is a monorepo that publishes two core packages to the NPM registry:

- `fumbly-cli-core`: The backend, handling logic and tool execution.
- `fumbly-cli`: The user-facing frontend.
...
- **NPM publication:** For publishing to the NPM registry, the TypeScript source code in `fumbly-cli-core` and `fumbly-cli` is transpiled into standard JavaScript using the TypeScript Compiler (`tsc`). The resulting `dist/` directory is what gets published in the NPM package. This is a standard approach for TypeScript libraries.
