...
This monorepo contains two main packages: `fumbly-cli` and `fumbly-cli-core`.

## `fumbly-cli`

This is the main package for the Fumbly AI CLI. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `fumbly-cli-core`. This means that whether a user installs the package with `npm install -g fumbly-cli` or runs it directly with `npx fumbly-cli`, they are using this single, self-contained executable.
